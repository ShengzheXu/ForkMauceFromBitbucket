...
class PyFunction {
    public:
        PyFunction(const std::string & filename, const std::string & functionName) {
            ...
            auto pName = PyString_FromString(const_cast<char*>(filename.data()));
            if (pName == NULL) {
                PyErr_Print();
...
f::Rewards arrayFromPyFunction(PyFunction & fun, const f::Action & arguments, const size_t retNum) {
    ...
    for (size_t i = 0; i < arguments.size(); ++i) {
        auto pValue = PyInt_FromLong(arguments[i]);
        if (!pValue) {
            Py_DECREF(pArgs);
...
constexpr auto maxPossiblePower = 14957.346067217015;
constexpr auto minPossiblePower = 12192.437539245086;//result[arguments.size()];
constexpr auto minPowerPerTurbine = 1659.5599978012335;
...
f::Rewards getRewards(const f::Action & arguments) {
    ...
    for (size_t i = 0; i < arguments.size(); ++i) {
        result[i] = (result[i] - minPowerPerTurbine) / (maxPossiblePower - minPowerPerTurbine * arguments.size());
...
inline auto makeTurbinesProblem() {
    ...
    constexpr double range = 0.02; // (5000.0 / 6.0) * 3.0;
